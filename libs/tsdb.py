...
def search_show(title):
    ...
    resp = api_utils.load_info(
        settings.SEARCH_URL, params=params, verboselog=settings.VERBOSELOG)
    if resp is not None:
        for league in resp.get('leagues'):
            if league.get('strLeague') == title:
                params['id'] = league.get('idLeague')
                resp_d = api_utils.load_info(
                    settings.SHOW_URL, params=params, verboselog=settings.VERBOSELOG)
...
def load_episode_info(show_id, episode_id):
    ...
    if show_info is not None:
        ...
        resp = api_utils.load_info(
            settings.EPISODE_URL, params=params, verboselog=settings.VERBOSELOG)
        if resp is None:
            return None
